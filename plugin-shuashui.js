...
bot.on("message.group", function (msg) {
	...
	if((m.indexOf("近20期") != -1 || m.indexOf("近 20 期") != -1) && (m.indexOf("291") != -1 || m.indexOf("292") != -1)){
		...
		if(null != qihao){
			...
			setTimeout(function() {
				http.get("http://121.4.87.215:8582/digital/digitalJndOnline/heartbeat?recordNumber="+qihao+"&group="+groupName,(res)=>{
				}).on("error",(e)=>{
			...
			getServerInfo("http://121.4.87.215:8582/digital/digitalJndOnline/getContent?recordNumber="+qihao+"&group="+groupName, 1, times, msg, groupName);
		}
	}
	...
	if(null != qihao && m.indexOf(qihao) != -1 && m.indexOf("总分") != -1 && m.indexOf("人") != -1){
		if(m.indexOf("汤姆") != -1){
			setTimeout(function() {
				...
				http.get("http://121.4.87.215:8582/digital/digitalJndOnline/sendScore?&recordNumber="+qihao+"&score="+b+"&group="+groupName,(res)=>{
				}).on("error",(e)=>{
					console.log(`获取数据失败: ${e.message}`)
					return;
				})
